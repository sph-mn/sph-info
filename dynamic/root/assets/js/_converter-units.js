...
module.define("ytilitu.converter.units", function() {
    var select2_options = {
        language: {
            searching: function() {
                return "searching...";
        ...
        ajax: {
            url: function(params) {
                return "/dynamic/ytilitu/json/converter/units/suggest-names/" + (params.term ? params.term : "");
            },
    ...
    var selects = $("#unit-from,#unit-to").select2(select2_options);
    var inputs = $("#value-from,#value-to");
    function unit_convert_ampersand(from, to, value, c) {
        var xhr = new XMLHttpRequest();
        xhr.open("get", encodeURI("/dynamic/ytilitu/json/converter/units/convert/" + from + "/" + to + "/" + value));
        xhr.onload = function() {
            if (200 === xhr.status) {
