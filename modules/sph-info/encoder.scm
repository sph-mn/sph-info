  ...
  (processor-routes "binary/text conversions" "/binary-text"
    ...
    (list "any" "base91"
      ...
      (l (request source-path target-path options)
        (call-with-output-file target-path
          (l (port)
            (false-if-exception (display (base91-encode (file->bytevector source-path)) port)))
          #:binary #t))
    ...
    (list "base91" "any"
      ...
      (l (request source-path target-path options)
        (call-with-output-file target-path
          (l (port)
            (false-if-exception
              (bytevector->file (base91-decode (file->string source-path)) target-path)))
          #:binary #t))
      (l (request file-name options) file-name) #f)))
