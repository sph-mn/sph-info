      ...
      ( (target-dir (ensure-trailing-slash directory)) (relative-dir "")
        (highlights
          (list-q (sph base91) (sph cli)
            (sph filesystem) (sph io path-pipe-chain)
            (sph lang indent-syntax) (sph lang parser type-signature)
            (sph lang plcss) (sph lang scm-format)
            (sph libmagic) (sph list)
            (sph process create) (sph process)
            (sph scgi) (sph scrypt)
            (sph server) (sph spline-path)
            (sph string) (sph test performance)
            (sph test) (sph thread-pool)
            (sph time gregorian) (sph time rfc3339)
            (sph time) (sph tree) (sph vector selection) (sph web atom)))
        (other
          (list-q (sph) (sph alist)
            ...
            (sph lang scm-format transform) (sph list other)
            (sph json) (sph log)
            (sph math) (sph module)
            (sph module binding-info) (sph number)
            (sph other) (sph server base)
            (sph server fibers) (sph sql)
            (sph stream) (sph system reader)
            (sph test base) (sph test report)
            (sph time stream) (sph time string)
            (sph time utc) (sph uniform-vector)
