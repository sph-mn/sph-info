(define-module (sph-info text))

(use-modules (rnrs sorting) (sph)
  (srfi srfi-1)
  ;(sph alist) (sph hashtable)
  ;(sph list) (sph pattern)
  (sph string)
  (sph vector)
  ;(sph web app) (sph web app client) (sph web app http) (sph web shtml)
  )
    ...
    (list-sort string<?
      (append
        (list "lowercase" "camelcase-to-dashes"
          ...
          "compress-whitespace-vertical" "randomize-lines")
        (filter-map
          (l (a)
    ...
    (l (request)
      (ytilitu-request-bind request (swa-env data route routes time-start)
        (respond-html request
          ...
          (q ytilitu) (list (list shtml-layout (shtml-operations route))) (cache-headers time-start))))))
